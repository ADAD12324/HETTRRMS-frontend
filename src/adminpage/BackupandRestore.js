...
export default function BackupandRestore() {
    ...
    axios.get('/backup')
      .then((response) => {
        if (response.data.success) {
          ...
          alert('Failed to create a backup');
        }
      })
      .catch((error) => {
        console.error(error);
  ...
  const handleRestore = (event) => {
    ...
    axios.post('/restore', formData)
      ...
      .catch((error) => {
        console.error(error);
        alert('An error occurred during database restoration');
      });
  };
  if (!showContent) {
    return <Loading />;
  ...
  <ListItem button key="Account" onClick={() => {navigate('/AdminProfile')}}>
    ...
    <ListItemText primary="Account" />
      ...
      <main className={`${classes.content} ${showContent ? 'fadeAnim' : ''}`}>
        <div className={classes.toolbar} />
        <div style={{textAlign:'center', border:'1px solid black', width:'40%', position:'relative', left:'30%', padding:'30px', marginTop:'10%', borderRadius:'20px'}}>
      <div className='bckres'>Backup and Restore</div>
      <button className='butback' onClick={handleBackup}>Manual Backup</button>
      <form onSubmit={handleRestore}>
        <input className='res' type="file" name="file" accept=".sql" /><br/>
        <button className='ressub' type="submit">Restore</button>
      </form>
    </div>
      </main>
    </div>
